...
inline fun inventory(
    lines: Int,
    noinline name: ((UUID) -> Component),
    crossinline apply: InventoryFactory.() -> Unit
): InventoryFactory {
...
inline fun dataInventoryBorder(
    lines: Int,
    noinline name: ((UUID) -> Component),
    crossinline apply: DataInventoryFactory.() -> Unit,
    material: Material = Material.GRAY_STAINED_GLASS_PANE
): DataInventoryFactory {
